...
namespace nim_comp
{
	...
	void UserService::LoginAppServer(const std::string &username, const std::string &password, const OnLoginAppServerCallback& cb)
	{
		std::string addressbook_address = kAppServerAddress;
		std::string new_addressbook_address = GetConfigValue(g_AppServerAddress);
		if (!new_addressbook_address.empty())
		...
		addressbook_address += "account/password_login";
		auto reg_cb = [cb](bool ret, int response_code, const std::string& reply)
		{
			if (ret && response_code == 200) {
				Json::Value json;
				...
				std::string  userId = json["data"]["userId"].asString();
				std::string  token = json["data"]["token"].asString();

				/*int json_res = json["res"].asInt();
				std::string err_msg = json["errmsg"].asString();*/

				nbase::ThreadManager::PostTask(kThreadUI, nbase::Bind(cb, code, userId, token, msg));
			}
			else {
				nbase::ThreadManager::PostTask(kThreadUI, nbase::Bind(cb, 0, "", "", nbase::UTF16ToUTF8(L"网络出现问题，请确认网络连接")));
			}
		...
		std::string privateKey = "ASDJIHDFA87T13";

		std::string signdata = appId + timeStamp + privateKey;
		Md5Encode encode_obj;
		std::string sign = encode_obj.Encode(signdata);
